...
3. **Admin**: cùng 1 codebase Next.js (route group), tham khảo bố cục từ **shadcn-admin** (mã nguồn mở)
   nhưng không chạy nó như 1 app riêng.
...
- **shadcn-admin chạy như app riêng** (Vite, tách biệt): loại bỏ vì tạo 2 hệ sinh thái frontend, 2 lần
  cấu hình bảng màu/theme, thêm 1 domain/container/pipeline — đi ngược tiêu chí "dễ kiểm soát, tránh
  lộn xộn, dễ bàn giao".

## Hệ quả
- (+) Tận dụng khung Clean Architecture đã kiểm chứng rộng rãi, giảm thời gian dựng khung ban đầu.
- (+) Frontend 1 codebase duy nhất, 1 bộ thiết kế nhất quán cho cả candidate/employer/admin.
- (−) Không có sẵn màn hình admin đầy đủ ngay lập tức như khi dùng thẳng shadcn-admin — phải tự build
  lại theo tham khảo, tốn công hơn một chút ở giai đoạn đầu.
