...
describe('useAuthStore', () => {
  ...
  it('persists tokens so a new store instance reads them back', async () => {
    ...
    expect(useAuthStoreAfterReload.getState().auth.accessToken).toBe('access-token')
    expect(useAuthStoreAfterReload.getState().auth.refreshToken).toBe('refresh-token')
  })
  ...
  it('reset clears user and tokens and drops persistence', async () => {
    const useAuthStore = await importAuthStore()
    useAuthStore.getState().auth.setTokens('will-be-cleared', 'refresh-cleared')
    useAuthStore.getState().auth.setUser({ ...sampleUser })
